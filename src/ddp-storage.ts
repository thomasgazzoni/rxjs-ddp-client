...
export abstract class DDPCacheEngine {
...
export class DDPStorage {
  ...
  setCacheEngine(cacheEngine: DDPCacheEngine) {
    ...
    this._cacheEngine
      .getItem(STORAGE_KEY_LAST_SYNC_TIME)
      .pipe(take(1))
      .subscribe(data => (this._lastSyncTime = data));
  }

  loadFromCache(collectionsNames: DDP_COLLECTIONS[]) {
    collectionsNames.forEach(collectionName => {
      ...
      this._cacheEngine
        ...
        .subscribe(data => {
          ...
          this._subjects[collectionName].next(
  ...
  persistToStore(collectionsNames: DDP_COLLECTIONS[]) {
    collectionsNames.forEach(collectionName => {
  ...
  clearCache(collectionsNames: DDP_COLLECTIONS[]) {
    this._lastSyncTime = undefined;
  ...
  removeItem(collectionName: DDP_COLLECTIONS, id: any) {
    ...
    this._collections[collectionName] = collection.filter(
      item => item._id !== id,
    );
  ...
  private _insertOrUpdate(collection: any[], newItem) {
  ...
  private _getCollection(collectionName: DDP_COLLECTIONS): any[] {
    this._checkAndInitCollection(collectionName);
